...
async function handleGetSequences(request, env) {
  const url = new URL(request.url);
...
async function handleCreateSequence(request, env) {
  try {
    const data = await request.json();
...
async function handleGetSequence(id, env) {
...
async function handleUpdateSequence(id, request, env) {
  try {
    const data = await request.json();
    ...
    // If activating sequence and it's set as welcome sequence, update the list
...
async function handleDeleteSequence(id, env) {
  const sequence = await env.DB.prepare('SELECT * FROM sequences WHERE id = ?').bind(id).first();
  if (!sequence) {
    return jsonResponse({ error: 'Sequence not found' }, 404);
  }
  ...
  // Remove as welcome sequence if set
  await env.DB.prepare(
...
async function handleGetSequenceSteps(sequenceId, env) {
...
async function handleAddSequenceStep(sequenceId, request, env) {
  try {
    const data = await request.json();
...
async function handleUpdateSequenceStep(sequenceId, stepId, request, env) {
  try {
    const data = await request.json();
...
async function handleDeleteSequenceStep(sequenceId, stepId, env) {
  const step = await env.DB.prepare(
    'SELECT * FROM sequence_steps WHERE id = ? AND sequence_id = ?'
  ).bind(stepId, sequenceId).first();
...
async function handleReorderSequenceSteps(sequenceId, request, env) {
  try {
    const data = await request.json();
...
async function handleEnrollInSequence(sequenceId, request, env) {
  try {
    const data = await request.json();
    ...
    // If email provided, find the subscription
...
async function handleGetSequenceEnrollments(sequenceId, request, env) {
  const sequence = await env.DB.prepare('SELECT * FROM sequences WHERE id = ?').bind(sequenceId).first();
  if (!sequence) {
    return jsonResponse({ error: 'Sequence not found' }, 404);
  }
