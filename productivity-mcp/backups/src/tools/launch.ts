...
export function registerLaunchTools(ctx: ToolContext) {
  ...
  server.tool("surface_launch_tasks", {
    ...
    if (candidates.results.length === 0) {
      return { content: [{ type: "text", text: "✅ No items to surface. All current phase items are either complete or already on your task list." }] };
    }
  ...
  server.tool("complete_checklist_item", {
    ...
    return { content: [{ type: "text", text: `✅ Completed: ${item.item_text}` }] };
  });
