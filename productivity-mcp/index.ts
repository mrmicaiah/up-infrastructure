...
function generateNudges(patterns: any[], openTasks: any[]): string[] {
    ...
    switch (pattern.pattern_type) {
      case 'peak_time':
        if (data.time === currentTime) {
          nudges.push(`Ã°Å¸â€Â¥ It's your peak time (${currentTime}) - tackle something important!`);
        }
        break;
      case 'peak_day':
        if (data.day === currentDay) {
          nudges.push(`Ã°Å¸â€œË† ${currentDay}s are your most productive day - make it count!`);
        }
        break;
      case 'avoidance_category':
        const avoidedTasks = openTasks.filter(t => t.category === data.category);
        if (avoidedTasks.length > 0) {
          nudges.push(`Ã¢Å¡Â Ã¯Â¸Â You have ${avoidedTasks.length} ${data.category} tasks piling up`);
        }
        break;
      case 'completion_rate_high':
        if (data.rate < 0.4) {
          nudges.push(`Ã°Å¸â€™Â¡ Try breaking down high-focus tasks - you complete ${Math.round(data.rate * 100)}% of them`);
        }
        break;
    }
  }
  ...
  if (upcoming.length > 0) {
    nudges.push(`Ã°Å¸â€”â€œÃ¯Â¸Â ${upcoming.length} task(s) due in the next 3 days`);
  }
  ...
  if (coldTasks.length >= 3) {
    nudges.push(`Ã¢Ââ€žÃ¯Â¸Â ${coldTasks.length} tasks are over a week old`);
  }
...
function registerTools(server: McpServer, agent: McpAgent, userId: string) {
  ...
  server.tool("list_tasks", {
    ...
    if (tasks.length === 0) {
      return { content: [{ type: "text", text: "No tasks found.\n\nÃ°Å¸â€™Â¬ What have you been working on?" }] };
    }
    
    let output = 'Found ' + tasks.length + ' tasks:\n\n';
    tasks.forEach((t: any) => {
      const p = t.priority >= 4 ? "Ã°Å¸â€Â´" : t.priority === 3 ? "Ã°Å¸Å¸Â¡" : "Ã¢Å¡Âª";
  ...
  server.tool("add_task", {
    ...
    recurrence: z.enum(['daily', 'weekdays', 'weekly', 'monthly', 'yearly']).optional(),
  }, async ({ text, priority, due_date, category, project, notes, for_user, recurrence }) => {
    const env = agent.env as any;
    ...
    await env.DB.prepare(
      'INSERT INTO tasks (id, user_id, text, priority, due_date, category, project, status, created_at, last_touched, needs_breakdown, is_vague, focus_level, notes, recurrence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).bind(id, targetUser, text, priority, due_date || null, category || null, project || null, 'open', ts, ts, needsBreakdown(text) ? 1 : 0, isVagueTask(text) ? 1 : 0, focusLevel, notes || null, recurrence || null).run();
    ...
    if (priority >= 4) resp += " Ã°Å¸â€Â´";
    if (due_date) resp += ' - due ' + due_date;
    if (recurrence) resp += ' 🔄 ' + recurrence;
    
    // Smart suggestions
    if (needsBreakdown(text)) {
      resp += '\n\nÃ°Å¸â€™Â¡ This looks like a big task. Want to break it down?';
    }
    if (isVagueTask(text)) {
      resp += '\n\nÃ°Å¸â€™Â­ This seems a bit vague. Can you make it more specific?';
    }
  ...
  server.tool("complete_task", {
    ...
    if (daysToComplete === 0) {
      resp += '\nÃ¢Å¡Â¡ Same-day completion!';
    } else if (daysToComplete <= 1) {
      resp += '\nÃ°Å¸Å½Â¯ Quick turnaround!';
    }
    
    if (checklistItem) {
      resp += '\nÃ°Å¸â€œâ€¹ Launch checklist item also marked complete';
    }
    ...
    if (task.recurrence) {
      ...
      await env.DB.prepare(
        'INSERT INTO tasks (id, user_id, text, priority, due_date, category, project, status, created_at, last_touched, needs_breakdown, is_vague, focus_level, notes, recurrence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      ).bind(newId, task.user_id, task.text, task.priority, nextDue, task.category || null, task.project || null, 'open', newTs, newTs, 0, 0, task.focus_level || 'medium', task.notes || null, task.recurrence).run();
  ...
  server.tool("log_progress", {
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸â€œÂ Logged: ' + description }] };
  });
  ...
  server.tool("get_daily_summary", {}, async () => {
    ...
    let output = 'Ã°Å¸â€œâ€¹ **Daily Summary - ' + getDayOfWeek().charAt(0).toUpperCase() + getDayOfWeek().slice(1) + '**\n\n';
    output += 'Ã¢Å“â€¦ Completed today: ' + (done?.c || 0) + '\n';
    output += 'Ã°Å¸â€œÂ Progress logged: ' + (progress?.c || 0) + ' entries\n';
    output += 'Ã°Å¸â€œÂ¬ Open tasks: ' + open.results.length + '\n';
    ...
    if (activeLaunch) {
      const daysToLaunch = activeLaunch.target_launch_date 
        ...
        : null;
      output += `\nÃ°Å¸Å¡â‚¬ **Active Launch:** ${activeLaunch.title}`;
  ...
  server.tool("analyze_patterns", {}, async () => {
    ...
    if (insights.length === 0) {
      return { content: [{ type: "text", text: "Ã°Å¸â€œÅ  Not enough data yet. Keep using the system and check back in a week!" }] };
    }
    
    let output = 'Ã°Å¸â€œÅ  **Your Productivity Patterns**\n\n';
    insights.forEach(i => { output += 'Ã¢â‚¬Â¢ ' + i + '\n'; });
  ...
  server.tool("get_insights", {}, async () => {
    ...
    if (patterns.length === 0) {
      return { content: [{ type: "text", text: "Ã°Å¸â€™Â¡ No patterns learned yet. Run analyze_patterns after a week of use!" }] };
    }
    
    let output = 'Ã°Å¸â€™Â¡ **Insights**\n\n';
    
    for (const pattern of patterns) {
      const data = JSON.parse(pattern.pattern_data);
      switch (pattern.pattern_type) {
        case 'peak_time':
          output += `Ã¢ÂÂ° You're most productive in the ${data.time}\n`;
          break;
        case 'peak_day':
          output += `Ã°Å¸â€œâ€¦ ${data.day.charAt(0).toUpperCase() + data.day.slice(1)}s are your power days\n`;
          break;
        case 'avg_completion_days':
          output += `Ã¢ÂÂ±Ã¯Â¸Â You complete tasks in ${data.days} days on average\n`;
          break;
        case 'avoidance_category':
          output += `Ã¢Å¡Â Ã¯Â¸Â You tend to delay ${data.category} tasks\n`;
          break;
      }
    }
  ...
  server.tool("end_of_day_recap", {}, async () => {
    ...
    if (nonMorningCheckpoints.length > 0) {
      out += `**Today's Flow:**\n`;
      const narrative = nonMorningCheckpoints.map((c: any) => c.summary).join(' â†’ ');
      out += narrative + '\n\n';
    }
    ...
    if (completed.results.length > 0) {
      ...
      for (const level of ['high', 'medium', 'low']) {
        if (focusLevels[level]) {
          const icon = level === 'high' ? 'ðŸ”´' : level === 'medium' ? 'ðŸŸ¡' : 'âšª';
    ...
    if (completed.results.length === 0 && checkpoints.results.length === 0 && progressLogs.results.length === 0) {
      ...
      out += `ðŸ’¡ Start tomorrow with "good morning" to track your work day!`;
    }
  ...
  server.tool("weekly_recap", {}, async () => {
    ...
    let recap = 'Ã°Å¸â€œÅ  Weekly Recap\n\nÃ¢Å“â€¦ Completed: ' + (done?.c || 0) + '\nÃ¢Å¾â€¢ Added: ' + (added?.c || 0);
  ...
  server.tool("plan_week", {
    ...
    if (peakDayPattern) {
      ...
      plan += '\nÃ°Å¸â€™Âª Your power day: ' + data.day + '\n';
    }
    ...
    if (activeLaunches.results.length > 0) {
      ...
      for (const launch of activeLaunches.results as any[]) {
        const daysToLaunch = launch.target_launch_date
          ...
          : null;
        plan += `Ã¢â‚¬Â¢ ${launch.title} - Phase: ${launch.current_phase}`;
    ...
    if (focus_level === 'low') {
      plan += '\nÃ¢Å¡Â Ã¯Â¸Â Low focus week - stick to quick wins and essentials only';
      const quickWins = tasks.results.filter((t: any) => t.focus_level === 'low');
      if (quickWins.length > 0) {
        plan += '\n\n**Quick wins available:**\n';
        quickWins.slice(0, 3).forEach((t: any) => { plan += 'Ã¢â‚¬Â¢ ' + t.text + '\n'; });
      }
    }
  ...
  server.tool("delete_task", { task_id: z.string() }, async ({ task_id }) => {
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸â€”â€˜Ã¯Â¸Â Deleted: "' + task.text + '"' }] };
  });
  ...
  server.tool("update_task", {
    ...
    return { content: [{ type: "text", text: "Ã¢Å“ÂÃ¯Â¸Â Updated" }] };
  });
  ...
  server.tool("snooze_task", {
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸ËœÂ´ Snoozed until ' + snoozeUntil + ': "' + task.text + '"' }] };
  });
  ...
  server.tool("get_challenges", {}, async () => {
    ...
    let out = 'Ã°Å¸Å½Â¯ **Challenges**\n\n';
    
    if (cold.results.length > 0) {
      out += 'Ã¢Ââ€žÃ¯Â¸Â **Cold Tasks** (' + cold.results.length + '):\n';
      cold.results.slice(0, 5).forEach((t: any) => { out += 'Ã¢â‚¬Â¢ ' + t.text + '\n'; });
      out += '\n';
    }
    
    if (vague.results.length > 0) {
      out += 'Ã°Å¸â€™Â­ **Vague Tasks** (' + vague.results.length + '):\n';
      vague.results.slice(0, 5).forEach((t: any) => { out += 'Ã¢â‚¬Â¢ ' + t.text + '\n'; });
      out += '\n';
    }
    
    if (needsBreaking.results.length > 0) {
      out += 'Ã°Å¸â€Â¨ **Needs Breakdown** (' + needsBreaking.results.length + '):\n';
      needsBreaking.results.slice(0, 5).forEach((t: any) => { out += 'Ã¢â‚¬Â¢ ' + t.text + '\n'; });
    }
    
    if (cold.results.length === 0 && vague.results.length === 0 && needsBreaking.results.length === 0) {
      out = 'Ã°Å¸Å½â€° No challenges right now! Your task list is in good shape.';
    }
  ...
  server.tool("team_summary", {}, async () => {
    ...
    let summary = 'Ã°Å¸â€˜Â¥ **Untitled Publishers Team**\n\n';
  ...
  server.tool("view_teammate_tasks", { 
    ...
    for (const tm of teammates) {
      ...
      out += `Ã°Å¸â€œâ€¹ **${tm}'s tasks** (${r.results.length}):\n`;
  ...
  server.tool("add_note", { title: z.string(), content: z.string().optional(), category: z.string().optional().default("General") }, async ({ title, content, category }) => {
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸â€œÂ Note saved: "' + title + '"' }] };
  });
  ...
  server.tool("add_idea", { title: z.string(), content: z.string().optional(), category: z.enum(["Writing Ideas", "Business Ideas", "Tech Ideas", "Content Ideas", "Unsorted"]).optional().default("Unsorted") }, async ({ title, content, category }) => {
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸â€™Â¡ Idea: "' + title + '"' }] };
  });
  ...
  server.tool("list_ideas", { category: z.string().optional() }, async ({ category }) => {
    ...
    let out = 'Ã°Å¸â€™Â¡ Ideas:\n';
    r.results.forEach((i: any) => { out += 'Ã¢â‚¬Â¢ ' + i.title + '\n'; });
    return { content: [{ type: "text", text: out }] };
  });
  ...
  server.tool("suggest_handoff", { 
    ...
    return { content: [{ type: "text", text: 'Ã°Å¸â€œÂ¤ Suggested to ' + targetTeammate + ': "' + task.text + '"' }] };
  });
  ...
  server.tool("accept_handoff", { task_id: z.string() }, async ({ task_id }) => {
    ...
    return { content: [{ type: "text", text: "Ã¢Å“â€¦ Accepted" }] };
  });
  ...
  server.tool("connection_status", {}, async () => {
    ...
    status += personalContactsToken ? 'âœ… Personal Contacts: Connected\n' : 'âŒ Personal Contacts: Not connected\n';
    status += companyContactsToken ? 'âœ… Company Contacts: Connected\n' : 'âŒ Company Contacts: Not connected\n';
  ...
  server.tool("drive_status", {}, async () => {
    ...
    if (!token) {
      ...
      return { content: [{ type: "text", text: 'Ã°Å¸â€â€” Not connected. Click:\n' + url }] };
    }
    return { content: [{ type: "text", text: "Ã¢Å“â€¦ Google Drive connected" }] };
  });

  server.tool("list_drive_folders", { parent_id: z.string().optional() }, async ({ parent_id }) => {
    const env = agent.env as any;
    const token = await getValidToken(env, getCurrentUser(), 'google_drive');
    if (!token) return { content: [{ type: "text", text: "Ã¢ÂÅ’ Not connected. Run: connect_service google_drive" }] };
    ...
    let out = 'Ã°Å¸â€œÂ Folders:\n';
    data.files.forEach((f: any) => { out += 'Ã¢â‚¬Â¢ ' + f.name + ' (ID: ' + f.id + ')\n'; });
    return { content: [{ type: "text", text: out }] };
  });
  ...
  server.tool("save_to_drive", { filename: z.string(), content: z.string(), folder_id: z.string().optional(), folder_path: z.string().optional() }, async ({ filename, content, folder_id, folder_path }) => {
    ...
    if (!resp.ok) return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error" }] };
    const file: any = await resp.json();
    return { content: [{ type: "text", text: 'Ã¢Å“â€¦ Saved: ' + file.name + '\n' + file.webViewLink }] };
  });
  ...
  server.tool("search_drive", { query: z.string() }, async ({ query }) => {
    ...
    let out = 'Ã°Å¸â€Â Results:\n';
    data.files.forEach((f: any) => { out += 'Ã¢â‚¬Â¢ ' + f.name + '\n  ' + f.webViewLink + '\n'; });
    return { content: [{ type: "text", text: out }] };
  });
  ...
  server.tool("check_inbox", {
    ...
    if (!token) {
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ ${account} email not connected. Run: connect_service ${provider}` }] };
    }
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error fetching emails" }] };
    }
    
    const data: any = await resp.json();
    
    if (!data.messages?.length) {
      return { content: [{ type: "text", text: `Ã°Å¸â€œÂ­ No unread emails in ${account} inbox` }] };
    }
    
    let out = `Ã°Å¸â€œÂ¬ **${account.charAt(0).toUpperCase() + account.slice(1)} Inbox** (${data.messages.length} unread):\n\n`;
  ...
  server.tool("read_email", {
    ...
    if (!token) {
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ ${account} email not connected` }] };
    }
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error fetching email" }] };
    }
  ...
  server.tool("search_email", {
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error searching emails" }] };
    }
    ...
    let out = `Ã°Å¸â€Â **Search results for "${query}"** (${data.messages.length}):\n\n`;
  ...
  server.tool("send_email", {
    ...
    if (!resp.ok) {
      ...
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ Error sending email: ${error}` }] };
    }
    
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Email sent to ${to}` }] };
  });
  ...
  server.tool("search_contacts", {
    ...
    if (!token) {
      return { content: [{ type: "text", text: `âŒ ${account} contacts not connected. Run: connect_service ${provider}` }] };
    }
    ...
    if (!resp.ok) {
      const error = await resp.text();
      return { content: [{ type: "text", text: `âŒ Error searching contacts: ${error}` }] };
    }
    ...
    let out = `ðŸ“‡ **Contacts matching "${query}"** (${account}):\n\n`;
    ...
    for (const result of data.results) {
      ...
      out += `â€¢ **${name}**\n`;
      if (email) out += `  ðŸ“§ ${email}\n`;
      if (phone) out += `  ðŸ“± ${phone}\n`;
      out += '\n';
    }
  ...
  server.tool("list_blogs", {}, async () => {
    ...
    if (!token) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Blogger not connected. Run: connect_service blogger" }] };
    }
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error fetching blogs" }] };
    }
    
    const data: any = await resp.json();
    
    if (!data.items?.length) {
      return { content: [{ type: "text", text: "Ã°Å¸â€œÂ No blogs found. Create one at blogger.com first." }] };
    }
    
    let out = 'Ã°Å¸â€œÂ **Your Blogs**\n\n';
  ...
  server.tool("list_blog_posts", {
    ...
    if (!token) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Blogger not connected" }] };
    }
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error fetching posts" }] };
    }
    ...
    let out = `Ã°Å¸â€œâ€ž **${status.charAt(0).toUpperCase() + status.slice(1)} Posts** (${data.items.length}):\n\n`;
    data.items.forEach((post: any) => {
      const date = new Date(post.published || post.updated).toLocaleDateString();
      out += `Ã¢â‚¬Â¢ **${post.title}**\n  ID: ${post.id}\n  Date: ${date}\n  URL: ${post.url || 'draft'}\n\n`;
    });
  ...
  server.tool("get_blog_post", {
    ...
    if (!resp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Error fetching post" }] };
    }
    
    const post: any = await resp.json();
    
    let out = `Ã°Å¸â€œâ€ž **${post.title}**\n\n`;
  ...
  server.tool("create_blog_post", {
    ...
    if (!resp.ok) {
      const error = await resp.text();
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ Error creating post: ${error}` }] };
    }
    
    const post: any = await resp.json();
    
    const status = is_draft ? 'Draft saved' : 'Published';
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ ${status}: "${post.title}"\n\nPost ID: ${post.id}\nURL: ${post.url || 'will be available after publishing'}` }] };
  });
  ...
  server.tool("update_blog_post", {
    ...
    if (!getResp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Post not found" }] };
    }
    ...
    if (!resp.ok) {
      const error = await resp.text();
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ Error updating post: ${error}` }] };
    }
    
    const post: any = await resp.json();
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Updated: "${post.title}"\n\nURL: ${post.url}` }] };
  });
  ...
  server.tool("publish_blog_post", {
    ...
    if (!resp.ok) {
      const error = await resp.text();
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ Error publishing: ${error}` }] };
    }
    
    const post: any = await resp.json();
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Published: "${post.title}"\n\nLive at: ${post.url}` }] };
  });
  ...
  server.tool("delete_blog_post", {
    ...
    if (!resp.ok) {
      const error = await resp.text();
      return { content: [{ type: "text", text: `Ã¢ÂÅ’ Error deleting: ${error}` }] };
    }
    
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Post deleted` }] };
  });
  ...
  server.tool("get_blog_stats", {
    ...
    if (!blogResp.ok) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Blog not found" }] };
    }
    ...
    if (postsResp.ok) {
      ...
      if (postsData.items?.length) {
        ...
        postsData.items.slice(0, 5).forEach((post: any) => {
          ...
          recentPosts += `Ã¢â‚¬Â¢ ${post.title} (${date})\n`;
        });
      }
    }
    ...
    let out = `Ã°Å¸â€œÅ  **${blog.name} Stats**\n\n`;
    out += `**Overview:**\n`;
    out += `Ã¢â‚¬Â¢ Total Posts: ${blog.posts?.totalItems || 0}\n`;
    out += `Ã¢â‚¬Â¢ Total Pages: ${blog.pages?.totalItems || 0}\n`;
    out += `Ã¢â‚¬Â¢ Page Views (${rangeLabel}): ${pageviews}\n`;
    out += `Ã¢â‚¬Â¢ URL: ${blog.url}\n`;
    out += recentPosts;
  ...
  server.tool("add_launch_doc", {
    ...
    if (phases.length === 0) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ No phases found. Use # PHASE 1: NAME format." }] };
    }
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Created: ${name}\n\nType: ${doc_type}\nPhases: ${phases.join(' Ã¢â€ â€™ ')}\nChecklist items: ${items.length}\nDoc ID: ${id}` }] };
  });
  ...
  server.tool("list_launch_docs", {
    ...
    if (r.results.length === 0) {
      return { content: [{ type: "text", text: "Ã°Å¸â€œâ€ž No launch documents found" }] };
    }
    
    let out = 'Ã°Å¸â€œâ€ž **Launch Documents**\n\n';
    r.results.forEach((doc: any) => {
      out += `Ã¢â‚¬Â¢ **${doc.name}** (${doc.doc_type})\n  ID: ${doc.id}\n  v${doc.version} | Updated: ${doc.updated_at.split('T')[0]}\n\n`;
    });
  ...
  server.tool("view_launch_doc", {
    ...
    if (!doc) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Document not found" }] };
    }
    
    const { phases, items } = parseLaunchDocument(doc.content);
    
    let out = `Ã°Å¸â€œâ€ž **${doc.name}**\n\n`;
    out += `Type: ${doc.doc_type} | Version: ${doc.version}\n`;
    out += `Phases: ${phases.join(' Ã¢â€ â€™ ')}\n`;
  ...
  server.tool("update_launch_doc", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Updated: ${existing.name}\n\nVersion: ${newVersion}\nPhases: ${phases.join(' Ã¢â€ â€™ ')}\nItems: ${items.length}` }] };
  });
  ...
  server.tool("create_launch", {
    ...
    for (const docId of launch_doc_ids) {
      ...
      if (!doc) {
        return { content: [{ type: "text", text: `Ã¢ÂÅ’ Document not found: ${docId}` }] };
      }
      docs.push(doc);
    }
  ...
  server.tool("launch_status", {
    ...
    if (!project) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ No launch project found" }] };
    }
    ...
    for (const section of sectionStats.results as any[]) {
      const icon = section.done === section.total ? 'Ã¢Å“â€œ' : section.done > 0 ? 'Ã¢â€”Â' : 'Ã¢â€”â€¹';
    ...
    if (recentMetrics.results.length > 0) {
      ...
      for (const m of recentMetrics.results as any[]) {
        ...
        if (!seen.has(key)) {
          out += `Ã¢â‚¬Â¢ ${m.metric_type}/${m.metric_name}: ${m.value}\n`;
          seen.add(key);
        }
      }
    }
  ...
  server.tool("launch_overview", {}, async () => {
    ...
    if (projects.results.length === 0) {
      return { content: [{ type: "text", text: "Ã°Å¸â€œâ€¹ No active launch projects" }] };
    }
    
    let out = 'Ã°Å¸â€œâ€¹ **All Active Launches**\n\n';
  ...
  server.tool("advance_launch_phase", {
    ...
    if (!project) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Project not found" }] };
    }
    ...
    if (criticalIncomplete.results.length > 0) {
      let out = `Ã¢ÂÅ’ Cannot advance. CRITICAL items incomplete:\n\n`;
      for (const item of criticalIncomplete.results as any[]) {
        out += `Ã¢â‚¬Â¢ ${item.item_text}\n`;
      }
      return { content: [{ type: "text", text: out }] };
    }
    ...
    if (currentIndex >= phaseList.length - 1) {
      return { content: [{ type: "text", text: "Ã¢Å“â€¦ Already on final phase. Use complete_launch when ready." }] };
    }
  ...
  server.tool("surface_launch_tasks", {
    ...
    if (candidates.results.length === 0) {
      return { content: [{ type: "text", text: "Ã¢Å“â€¦ No items to surface. All current phase items are either complete or already on your task list." }] };
    }
    ...
    for (const item of candidates.results as any[]) {
      ...
      surfaced.push(`Ã¢â‚¬Â¢ ${item.item_text}${tagStr}`);
    }
    
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Surfaced ${surfaced.length} items to your task list:\n\n${surfaced.join('\n')}` }] };
  });
  ...
  server.tool("complete_checklist_item", {
    ...
    if (!item) {
      return { content: [{ type: "text", text: "Ã¢ÂÅ’ Item not found" }] };
    }
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Completed: ${item.item_text}` }] };
  });
  ...
  server.tool("list_checklist", {
    ...
    let out = `Ã°Å¸â€œâ€¹ **Checklist** (${r.results.length} items)\n\n`;
    let currentPhase = '';
    let currentSection = '';
    ...
    for (const item of r.results as any[]) {
      ...
      const check = item.completed ? 'Ã¢Å“â€œ' : 'Ã¢â€”â€¹';
      const tags = JSON.parse(item.tags || '[]');
      const tagStr = tags.length > 0 ? ` [${tags.join(', ')}]` : '';
      const onTask = item.task_id ? ' Ã°Å¸â€œÅ’' : '';
  ...
  server.tool("add_checklist_item", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Added: ${item_text}` }] };
  });
  ...
  server.tool("log_launch_metrics", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Logged metrics:\n\n${logged.join('\n')}` }] };
  });
  ...
  server.tool("launch_metrics_history", {
    ...
    let out = `Ã°Å¸â€œÅ  **Metrics History** (${days} days)\n\n`;
    let currentDate = '';
  ...
  server.tool("log_content_batch", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Logged batch:\n\nScripted: ${scripted}\nFilmed: ${filmed}\nEdited: ${edited}\nScheduled: ${scheduled}\n\nÃ°Å¸â€œÂ¦ Content buffer: ~${bufferDays} days` }] };
  });
  ...
  server.tool("log_post", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Logged ${count} ${platform} post(s)\n\nÃ°Å¸â€Â¥ Current streak: ${streak} days` }] };
  });
  ...
  server.tool("posting_streak", {
    ...
    let out = 'Ã°Å¸â€Â¥ **Posting Streaks**\n\n';
  ...
  server.tool("launch_checkin", {
    ...
    return { content: [{ type: "text", text: `Ã¢Å“â€¦ Week ${weekNumber} check-in recorded!\n\nÃ°Å¸â€Â¥ Check-in streak: ${checkinCount?.count || 1} weeks` }] };
  });
  ...
  server.tool("checkin_history", {
    ...
    let out = 'Ã°Å¸â€œâ€¹ **Check-in History**\n\n';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname.startsWith('/api/')) {
      const apiRoutes = createApiRoutes(env as any);
    ...
    if (url.pathname === "/oauth/callback") {
      ...
      return new Response(`<html><body style="font-family:system-ui;display:flex;justify-content:center;align-items:center;height:100vh"><div style="text-align:center"><h1>Ã¢Å“â€¦ ${serviceNames[provider] || provider} Connected!</h1><p>Close this window and return to Claude</p></div></body></html>`, { headers: { 'Content-Type': 'text/html' } });
    }
